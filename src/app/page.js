...
export default function Home() {
  ...
  const [visibleVideos, setVisibleVideos] = useState(12); // Carregar 12 por vez
  const [loadedVideos, setLoadedVideos] = useState(new Set()); // Rastrear vídeos carregados
  ...
  // Extrair ID do YouTube do embed URL
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      {/* Background animado futurista - OTIMIZADO */}
      ...
      {/* Particles effect - REDUZIDO para performance */}
      ...
      <div className={`transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="max-w-7xl mx-auto">
              ...
              {videos.slice(0, visibleVideos).map((video, idx) => (
                ...
                >
                  ...
                  <div className="relative bg-white/5 backdrop-blur-xl rounded-3xl p-6 border border-white/10 overflow-hidden transition-all duration-500 hover:bg-white/10 hover:border-cyan-500/50 hover:shadow-[0_0_50px_rgba(6,182,212,0.3)] hover:-translate-y-2">
                    ...
                    {/* Container do vídeo - SUPER OTIMIZADO COM THUMBNAIL */}
                    <div className="relative aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 shadow-2xl">
                      {!loadedVideos.has(idx) ? (
                        // THUMBNAIL - Carrega apenas imagem (muito mais leve!)
                        <div 
                          className="relative w-full h-full cursor-pointer group/thumb"
                          onClick={() => handleVideoClick(idx)}
                        >
                          {/* Thumbnail do YouTube (imagem estática) com fallback inteligente */}
                          <img 
                            src={`https://img.youtube.com/vi/${getYouTubeId(video.embed)}/sddefault.jpg`}
                            alt={video.title}
                            className="w-full h-full object-cover"
                            loading="lazy"
                            onError={(e) => {
                              // Fallback em cascata: sd → hq → mq → default
                          ...
                          {/* Overlay escuro */}
                          <div className="absolute inset-0 bg-black/40 group-hover/thumb:bg-black/20 transition-all duration-300"></div>
                          
                          {/* Botão PLAY grande */}
                          <div className="absolute inset-0 flex items-center justify-center">
                            <div className="w-20 h-20 bg-red-600/90 rounded-full flex items-center justify-center group-hover/thumb:scale-110 group-hover/thumb:bg-red-500 transition-all duration-300 shadow-2xl">
                          ...
                          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/70 px-4 py-2 rounded-full text-white text-sm font-semibold backdrop-blur-sm">
                            🎬 Clique para assistir
                          </div>
                        </div>
                      ) : (
                        // IFRAME - Carrega apenas quando clicado
                        <iframe
                          width="100%"
                          height="100%"
                      ...
                      {/* Botão para abrir no YouTube - sempre visível */}
                      <a 
                        href={video.url} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="absolute top-3 right-3 bg-red-600/90 hover:bg-red-500 text-white px-3 py-2 rounded-xl text-xs font-semibold backdrop-blur-sm transition-all duration-300 hover:scale-110 flex items-center gap-2 shadow-lg z-10"
            ...
            {visibleVideos < videos.length && (
              <div className="mt-12 text-center">
                <button
                  onClick={loadMore}
                  className="group relative px-8 py-4 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-2xl overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-[0_0_40px_rgba(6,182,212,0.5)]"
                >
                  ...
                  <span className="text-white font-bold relative z-10 flex items-center gap-2">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                    Carregar Mais Vídeos ({videos.length - visibleVideos} restantes)
                  </span>
                </button>
              </div>
            )}
        ...
        {/* Footer */}
        <footer className="relative z-10 py-12 px-6 border-t border-white/10 backdrop-blur-xl mt-12">
          <div className="max-w-6xl mx-auto text-center">
            <div className="mb-6">
            ...
            <div className="flex justify-center gap-6 text-sm">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="text-gray-400 hover:text-green-400 transition-all duration-300 flex items-center gap-2 group"
              >
                <svg className="w-5 h-5 transition-transform group-hover:scale-110" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
